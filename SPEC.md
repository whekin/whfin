...
|---|---|---|
| SMS банка (Credo) | реальное время, черновик; явный resolver одновременно проверяет выбранную операцию | `pending` → `confirmed` после review |
...
- Начиная с Room DB v2 все изменения схемы выполняются явными миграциями; текущая схема — v7. Destructive
