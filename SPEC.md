...
- **Account**: name, type (`BANK | CASH | SAVINGS | CRYPTO | PERSON`), currency, iban?, isArchived; card masks отдельной таблицей.
  ...
  savingsMode меняются для них атомарно, но баланс и история всегда остаются раздельными по валюте
  Cash использует ту же UI-грамматику source→account→currency: alias опционален, а savingsMode позволяет
  считать конкретный cash-ledger доступными деньгами или резервом
...
6. **Накопления**: SAVINGS-счета с опциональной целью; переводы туда = «отложено», не расход
