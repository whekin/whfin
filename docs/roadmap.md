...
- [x] Код и экранная блокировка разведены как два разных решения. `SensitiveActionController`
  переспрашивает код перед выгрузкой копии, восстановлением, сохранённым входом MyCredo и самой
  настройкой блокировки; он живёт от наличия кода, а не от таймаута. Экран «Блокировка WHFIN» ведёт
  разделом `ЧЕМ ПОДТВЕРЖДАТЬ` (задать/изменить код, биометрия), задержка идёт вторым разделом.
  Раньше код нельзя было завести иначе как выбрав ненулевую задержку, поэтому «код без блокировки
  экрана» существовал, но был недостижим без обхода в два шага. Строка Настроек несёт состояние кода
  (`Код не задан` / `Код задан · без блокировки экрана` / `Код задан · Через 1 минуту`), а не только
  задержку: с кодом и выключенной блокировкой она читалась как `Выключено`.
