...
- `WhfinDock`: one grounded, full-width primary rail with two stable destinations and a separate create action. Destinations use 22 dp outlined glyphs, restrained labels, 48 dp-or-larger targets, and a short local ledger rule for selection; they never gain a pill or move. The center create action is an unfilled icon-and-short-label button aligned to the same 64 dp rhythm. Its custom plus uses a 2.5 dp rounded stroke so its optical weight matches the outlined destination glyphs. It uses the primary color but no selection rule: it is a button, not a third destination, and keeps a 48 dp-or-larger target without obscuring ledger rows.
