...
/**
 * Где деньги входят в мир пользователя.
 *
 * Это не план платежа: подтверждать здесь нечего, факт по-прежнему приходит из банка или из
 * блокчейна. Объявляется одно — что зарплата такого размера приходит вот на этот счёт. Из этого
 * следует правило, которое иначе приходится угадывать эвристиками: **всё, что происходит после
 * точки входа, — движение денег, а не доход**. Обмен, внесение в кассу, перевод на депозит не могут
 * быть доходом по построению, и одни и те же деньги не считаются дважды.
 *
 * [expectedDayFrom] — обычное число выплаты, [expectedDayTo] — редкий крайний срок. Это не
 * равновероятное окно: прогноз должен вести обычной датой и показывать край отдельно. Если обычное
 * число выпало на выходной, календарный ориентир сдвигается к следующему буднему дню; ранний приход
 * остаётся приятным фактом, а не обещанием. Первая выплата на новом месте вообще может не попасть
 * ни в какой шаблон.
 *
 * [startedOn]/[endedOn] существуют потому, что источник дохода за жизнь меняется — наличные,
 * обменник, крипта. Объявление действует вперёд и не переписывает историю: закончить одно и начать
 * другое честнее, чем задним числом объявить, что прошлое было иным.
 */
@Entity(
...
data class IncomeSourceEntity(
    ...
    /** Валюта объявления; может отличаться от валюты счёта (2700 USDT на кошелёк). */
    val currency: String,
...
data class CounterpartyRuleEntity(
    ...
    /**
     * Когда пользователь сказал, что про этот счёт спрашивать не надо.
     *
     * Большинство переводов человеку разовые и между собой не связаны: общая категория для них
     * была бы выдумкой. Отказ поэтому такое же решение, как и категория, и хранится рядом с ней —
     * иначе строка возвращалась бы в список после каждого импорта, а «нет» было бы невыразимо.
     */
