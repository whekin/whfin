...
In multi-cell variants the add action retains a 48 dp interactive slot, but its colored surface is inset to a 40 dp circle. The visible action must not merge with the full-height widget bar; the 4 dp surround keeps it distinct without reducing accessibility, so do not add another divider before it. In the 2-cell composition only, the source label is nudged 10 dp toward the visually heavy trailing action; wider source/currency segments remain geometrically centered. The standalone 1-cell variant remains a 46 dp filled circle. Quick expense is a real Material modal bottom sheet: its drag handle, downward swipe, scrim tap, and explicit close action all dismiss the translucent host activity. The Compose sheet owns the only dim scrim; window-level dimming is forbidden because it survives until Activity finish and makes the launcher react one beat late. Scrim alpha follows the sheet's actual drag offset rather than its binary Expanded/Hidden target. Square the drag fraction before applying smoothstep: this retains about 58% of the initial scrim after two thirds of the travel, reveals the background fully only at the hidden anchor, and settles with zero alpha velocity at both ends. Quick expense uses its compact calculator keypad by default so OEM numeric keyboard toolbars cannot consume the entry surface. Settings → Appearance can disable it; that mode turns the amount into an auto-focused decimal field and intentionally uses the device IME. The system text IME is always used for the optional description. Calculator operations live in one six-action rail above a strict three-column numeric grid; the final row is decimal, zero, and Backspace. Do not reproduce that layout with nested weighted rows because their column widths drift.
