...
- [x] Data Safety: destructive Room fallback удалён; v1→v2 оформлена явной migration, которая сохраняет
  ...
  Home молчит, пока данные не противоречат себе; всё остальное состояние (черновики, unrouted SMS,
  дата последней выписки, исправления, архивные счета) собрано в `Состояние данных`. Проверка
