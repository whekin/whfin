...
- [x] Умные категории без backend: Settings показывает покрытие по активным расходам, запускает
  ...
  Портируемый бэкап переносит добавленную колонку через `defaultsOnRestore`: копия, снятая неделю
  назад, описывает полный ledger, и новая колонка не делает её неполной. Прежний частный случай
  `isPrimary` выражен тем же механизмом. Колонки с деньгами в список не попадают. Шаги старта изолированы друг от друга: они делили одну
