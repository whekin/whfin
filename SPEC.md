...
- Для личной dogfood-сборки разрешён отдельный foreground-коннектор к текущему веб-протоколу MyCredo:
  ...
  сверки. Через 30 дней Home показывает одну спокойную строку `Сверить Credo`: сколько дней прошло и
  сколько активных SMS-операций ещё ждут подтверждения выпиской. Это не пользовательская очередь
  подтверждения; тап ведёт в обычный login/OTP → sync flow. Догрузка старой истории остаётся отдельным
  редким действием и не подменяет регулярную сверку.
