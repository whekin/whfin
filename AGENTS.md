...
- [x] Data Safety: destructive Room fallback удалён; v1→v2 оформлена явной migration, которая сохраняет
  ...
  требует `UNCHANGED`, а не «нечего вставлять». MyCredo sync намеренно без preview: файлы приходят
  сами после аутентифицированного входа, ошибиться файлом нельзя, ledger на каждый валютный счёт —
  желаемый результат, а ежедневный sync почти всегда ничего не вставляет. Проверено на disposable
