...
- [x] Data Safety: destructive Room fallback удалён; v1→v2 оформлена явной migration, которая сохраняет
  ...
  `OpeningAnchor`, `TransferPairing`. Появился `preview()` — что файл изменил бы, без записи и без
  создания счёта; в UI импорта пока не подключён. Черновик может подтвердить только одна строка
