...
- [~] Savings projection: чистый сценарный расчёт `projectSavings` покрыт JVM-тестами (темп,
  дата цели, сумма к сроку, високосный февраль, переполнение). Первое равное пополнение — через месяц
  от текущей даты; проценты/снятия не выдумываются, в ledger ничего не пишется. Далее: календарь и live UI.
