...
- `WhfinDock`: one grounded, full-width primary rail with two stable destinations and a separate create action. Destinations use 22 dp outlined glyphs, restrained labels, 48 dp-or-larger targets, and a short local ledger rule for selection; they never gain a pill or move. The center create action is a flat, filled 48 dp primary circle raised only 4 dp inside its 64 dp slot—never an outlined ring, floating capsule, notch, or shadow. Its strong fill is reserved for creation and must not obscure ledger rows.
